...
- **專案簡介：**
  ...
  可以切換年度、篩選組別，或是依進度、瀏覽數和今日發文排序。目前收錄 2026 年的 127 支系列、17 個組別（2026 年 8 月 6 日抓取）。
...
我把進度、最新文章、瀏覽數、組別和作者放在同一張卡片上。我自己最常用的是「今日發文」排序，開網站先看今天誰有更新，再瞄一眼誰快完賽或已完賽。
...
- **參賽進度：** 看哪些系列走得比較前面
- **最多觀看：** 看文章累計瀏覽數較高的系列
- **今日發文：** 今天有發文的系列先出現，同一天再依發布時間排序，沒有文章的系列排在最後
...
抓 127 支系列時，遇到連線失敗、暫時性 403 或頁面格式改變都很正常。如果任何一支失敗就讓整次程式停止，最後很可能一筆新資料都沒有。
